...
bool f::players::get_data(int32_t idx, c_cs_player_controller* player, c_cs_player_pawn* player_pawn)
{
	...
	// Highlight local player as GREEN
	int color = player->get_color();
	if (player == sdk::m_local_controller)
		color = static_cast<int>(e_colors::green);
...
void f::players::get_weapons(c_cs_player_pawn* player_pawn)
{
	...
	{
		const auto weapon = my_weapons.m_elements->get(static_cast<int32_t>(idx));
		if (!weapon) continue;
...
void f::players::get_active_weapon(c_cs_player_pawn* player_pawn)
{
	...
	if (!weapon_handle.is_valid()) return;
